    ...
    while not swapped:
        ...
        if random_node != chosen_value:
            ...
            if sym_classes is not None:
                ...
                # go through the sym classes and check if more than one of the displayed nodes is in each class
                for sym_class in sym_classes:
                    ...
                    if len(displayed_in_class) > 1:
                        logger.debug(f"Symmetry class {sym_class} ")
...
class SimulatedAnnealing2:
    def __init__(self, json_data):
        ...
        self.possible_symmetry = json_data["oldjsondata"]["possible_symmetry"]

        # self.orig_nodes_displayed = json_data["oldjsondata"].get("orig_nodes_displayed", [])
        self.orig_nodes_displayed = json_data["oldjsondata"]["orig_nodes_displayed"]

        logger.debug(f"Original nodes displayed: {self.orig_nodes_displayed}")
        ...
        logger.debug(f"Symmetry classes: {self.sym_classes}")
