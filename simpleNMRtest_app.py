...
def index():
    """Serve the main documentation index at /documentation/."""
    return send_from_directory(DOCS_DIR, "index.html")


@app.route("/documentation/<path:filename>")
def docs(filename):
    """Serve other static files (CSS, JS, images, etc.) under /documentation/."""
...
def check_machine_learning():
    """
    Check if the user has opted in for machine learning.
    :return: JSON response indicating the user's consent status.
    """
...
def registration_page():
    """
    Serve the registration page with hostid as a query parameter.
    Query format: /registration?hostid=VALUE
    """
