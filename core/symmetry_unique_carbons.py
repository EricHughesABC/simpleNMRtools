...
def get_carbon_symmetry_classes(mol, use_chirality=False):
    """
    ...
    ----------
    use_chirality : bool
        If True, folds defined stereochemistry into the ranking
        (Chem.AssignStereochemistry + CanonicalRankAtoms(includeChirality=True)).
        NOTE: this only splits atoms close enough to directly "see" a
        defined stereocenter in RDKit's CIP comparison -- it does NOT catch
        the common case of diastereotopic groups one or more bonds away from
        a stereocenter (e.g. isopropyl methyls next to a distal stereocenter,
        the classic valine/leucine case), which pure topology fundamentally
        cannot see without a more involved substitution-test analysis or
        explicit experimental input. See split_symmetry_class() for a manual
        override driven by experimental NMR data.
    """
    mol_for_ranking = mol
    if use_chirality:
        mol_for_ranking = Chem.Mol(mol)
        Chem.AssignStereochemistry(mol_for_ranking, cleanIt=True, force=True)

    ranks = list(
        Chem.CanonicalRankAtoms(mol_for_ranking, breakTies=False, includeChirality=use_chirality)
    )
...
def split_symmetry_class(carbon_classes, atoms_to_split):
    """
    Manually split specific atoms out of their current symmetry class into
    their own singleton class(es).

    Use this when experimental NMR data (e.g. more distinct DEPT/HSQC peaks
    than predicted for a given carbon type) reveals that atoms currently
    grouped together as symmetry-equivalent are actually chemically distinct
    (most commonly: diastereotopic groups near a stereocenter, which pure
    topological symmetry cannot detect on its own -- see get_carbon_symmetry_classes).

    This is a targeted, manual refinement: only the specified atoms are
    pulled out; every other class is left untouched. The result can be fed
    straight back into greedy_group_steiner_tree / get_minimal_unique_carbon_set /
    enumerate_all_symmetric_combinations exactly like any other class list --
    singleton classes are already handled as mandatory inclusions everywhere
    downstream, so no other code needs to change.

    Parameters
    ----------
    carbon_classes : list of lists
        Existing symmetry classes (from get_carbon_symmetry_classes).
    atoms_to_split : iterable of int
        Atom indices that should each become their own singleton class,
        removed from whatever multi-member class currently contains them.
        (If an atom isn't found in any class, it's silently ignored.)

    Returns
    -------
    new_classes : list of lists
        Updated class list, in the same format as get_carbon_symmetry_classes.
    """
    atoms_to_split = set(atoms_to_split)
    new_classes = []
    for cls in carbon_classes:
        remaining = [a for a in cls if a not in atoms_to_split]
        if remaining:
            new_classes.append(remaining)
    found = set()
    for cls in carbon_classes:
        found.update(a for a in cls if a in atoms_to_split)
    for a in sorted(found):
        new_classes.append([a])
    return new_classes


def carbon_type(mol, atom_idx):
    """Classify a carbon by attached-H count, for cross-referencing against DEPT/HSQC data."""
    n_h = mol.GetAtomWithIdx(atom_idx).GetTotalNumHs()
    return {0: "C (quaternary)", 1: "CH", 2: "CH2", 3: "CH3"}.get(n_h, f"CH{n_h}?")


def reconcile_symmetry_with_experimental_counts(
    mol, carbon_classes, experimental_counts, pool_ch3_ch1=False, verbose=True, molgraph=None
):
    """
    Loosen symmetry classes of any carbon type where the experimental peak
    count exceeds the topologically-predicted count -- i.e. where pure graph
    symmetry is too aggressive relative to what NMR actually shows for that
    type.

    For a type/group showing a deficit, this opens only as many atoms as
    needed to cover the deficit, and -- critically -- prefers classes that
    have an internal "same-parent" cluster of 2+ atoms (e.g. both methyls of
    one gem-dimethyl group, which share the same neighboring carbon and are
    therefore guaranteed to sit on the same physical side/arm) over classes
    with no such internal redundancy (e.g. one representative per physical
    ring/arm, like a pair of angular methyls on two different ring-fusion
    carbons -- opening these unavoidably requires an atom from each side).

    When a class with a same-parent cluster is opened, only that cluster's
    atoms are used (as singletons) -- any other members of that original
    class (e.g. the OTHER ring's copy of the same gem-dimethyl pair) are
    simply discarded, not kept as a separate merged class. This is valid
    because all members of the original class were mutually equivalent by
    symmetry in the first place, so any same-side subset is an equally
    correct choice of representative(s) -- discarding the rest doesn't lose
    anything a downstream matching step would have needed.

    Only when no same-parent cluster is available (or isn't enough to cover
    the remaining deficit on its own) does this fall back to a class with no
    internal redundancy, which necessarily pulls in an atom from each side.

    A carbon type with no deficit is left completely untouched.

    Parameters
    ----------
    mol : rdkit.Chem.Mol
    carbon_classes : list of lists
        From get_carbon_symmetry_classes(mol).
    experimental_counts : dict
        Maps carbon type label ("C (quaternary)", "CH", "CH2", "CH3" -- same
        labels as carbon_type()) -> number of distinct experimental peaks
        observed of that type. Omit a type (or use 0) if no data exists yet.
    pool_ch3_ch1 : bool
        Set True when there is no experimental evidence distinguishing CH3
        from CH1 (e.g. the CH3/CH1 assignment chain fell through to the
        shift/67ppm-cutoff fallback) -- CH3 and CH1 classes/counts are then
        compared and loosened together, never separately.
    verbose : bool
        Print a log line for every carbon type/group that gets loosened,
        including how many classes were opened vs. left merged, and whether
        the loosening fully resolved the deficit or a genuine (physically
        unresolvable) shortfall remains.
    molgraph : nx.Graph, optional
        Used to detect same-parent clusters. Built from `mol` if not given.

    Returns
    -------
    new_classes : list of lists
        Updated carbon symmetry classes.
    loosened_log : list of dict
        One entry per carbon type/group that needed loosening:
        {'group': ..., 'predicted_before': ..., 'predicted_after': ...,
         'experimental_count': ..., 'classes_opened': ...,
         'classes_still_merged': ..., 'still_short': bool}
        `still_short=True` means even opening every available class in that
        group's symmetry wasn't enough -- this is the genuine-mismatch case
        that should still hit the existing hard-stop/error-table path
        unchanged.
    """
    if molgraph is None:
        molgraph = build_molgraph(mol)

    def same_parent_clusters(cls):
        """
        Group a class's members by shared immediate neighbor ("parent").
        Members sharing a parent (e.g. both methyls of one gem-dimethyl
        group) are guaranteed to sit on the same physical side, unlike
        members with different parents (e.g. one representative per
        physical ring/arm). Atoms with no shared-parent sibling form their
        own singleton cluster.
        """
        groups = defaultdict(list)
        singles = []
        for atom in cls:
            neighbors = list(molgraph.neighbors(atom))
            if len(neighbors) == 1:
                groups[neighbors[0]].append(atom)
            else:
                singles.append(atom)
        clusters = list(groups.values())
        clusters.extend([a] for a in singles)
        return clusters

    class_types = []
    for cls in carbon_classes:
        types_in_class = set(carbon_type(mol, a) for a in cls)
        assert len(types_in_class) == 1, (
            f"Class {cls} has mixed carbon types: {types_in_class} -- "
            "this should never happen from get_carbon_symmetry_classes"
        )
        class_types.append(types_in_class.pop())

    def group_key(t):
        if pool_ch3_ch1 and t in ("CH", "CH3"):
            return "CH+CH3 (pooled, no experimental evidence to distinguish)"
        return t

    indices_by_group = defaultdict(list)
    for ci, t in enumerate(class_types):
        indices_by_group[group_key(t)].append(ci)

    exp_by_group = defaultdict(int)
    for t, count in experimental_counts.items():
        exp_by_group[group_key(t)] += count

    touched_class_indices = set()
    replacement_singletons = []
    loosened_log = []

    all_groups = set(indices_by_group) | set(exp_by_group)
    for group in all_groups:
        class_indices = indices_by_group.get(group, [])
        predicted_before = len(class_indices)
        experimental = exp_by_group.get(group, predicted_before)  # no data -> assume no deficit

        if experimental > predicted_before:
            deficit = experimental - predicted_before

            # Prefer classes with an internal same-parent cluster of size >= 2
            # (same-side redundancy available) over classes with none (every
            # member on a different side/arm, no same-side alternative).
            # Within each preference tier, prefer smaller classes first, to
            # minimize how much gets pulled in beyond the deficit.
            def priority_key(ci):
                clusters = same_parent_clusters(carbon_classes[ci])
                best_cluster_size = max(len(c) for c in clusters)
                has_same_side_option = best_cluster_size >= 2
                return (not has_same_side_option, len(carbon_classes[ci]))

            multi_member = sorted(
                (ci for ci in class_indices if len(carbon_classes[ci]) > 1),
                key=priority_key,
            )

            remaining = deficit
            opened = []  # (class_idx, kept_atoms)
            for ci in multi_member:
                if remaining <= 0:
                    break
                cls = carbon_classes[ci]
                clusters = sorted(same_parent_clusters(cls), key=len, reverse=True)
                needed = remaining + 1  # total representatives this class must contribute

                if len(clusters[0]) >= needed:
                    # the single largest same-side cluster alone covers what
                    # this class needs to contribute -- use only `needed` of
                    # its members; any other members (e.g. the other side's
                    # copy of this same gem-dimethyl-like pair) are discarded
                    # entirely, not kept as a separate merged class, since
                    # they were chemically equivalent to the kept ones anyway
                    kept = clusters[0][:needed]
                else:
                    # no single same-side cluster is big enough on its own;
                    # fall back to using the whole class (all clusters)
                    kept = list(cls)

                opened.append((ci, kept))
                remaining -= len(kept) - 1

            for ci, kept in opened:
                touched_class_indices.add(ci)
                replacement_singletons.extend([atom] for atom in kept)

            predicted_after = predicted_before + sum(len(kept) - 1 for _, kept in opened)
            still_short = remaining > 0
            ...
            loosened_log.append(
                {
                    ...
                    "classes_opened": len(opened),
                    "classes_still_merged": predicted_before - len(opened),
                    "still_short": still_short,
                }
            )
            if verbose:
                status = (
                    "STILL SHORT -- genuine mismatch, needs manual review"
                    if still_short
                    else "resolved by loosening"
                )
                print(
                    f"Symmetry loosened for {group}: {predicted_before} -> "
                    f"{predicted_after} predicted (experimental={experimental}), "
                    f"opened {len(opened)} class(es), left "
                    f"{predicted_before - len(opened)} still merged [{status}]"
                )

    new_classes = [
        cls for i, cls in enumerate(carbon_classes) if i not in touched_class_indices
    ] + replacement_singletons
    return new_classes, loosened_log


def print_class_summary(mol, carbon_classes, chosen_reps=None):
    """
    Print every carbon symmetry class with its size, carbon type (CH3/CH2/CH/
    quaternary), and 1-indexed atom numbers (matching molfile numbering) --
    designed to make it easy to cross-reference against experimental
    DEPT/HSQC peak counts and decide which classes (if any) need manual
    splitting via split_symmetry_class, e.g. when experimental data shows
    more distinct peaks of a given type than predicted.

    Parameters
    ----------
    chosen_reps : dict, optional
        class_idx -> representative atom idx (e.g. from greedy_group_steiner_tree),
        included in the printout if provided.
    """
    print("Carbon symmetry classes (1-indexed atom numbers):")
    for ci, cls in enumerate(carbon_classes):
        types_in_class = set(carbon_type(mol, a) for a in cls)
        type_label = types_in_class.pop() if len(types_in_class) == 1 else f"MIXED {types_in_class}"
        atoms_1indexed = sorted(a + 1 for a in cls)
        flag = "  <-- inconsistent H-count within class!" if "MIXED" in type_label else ""
        rep_str = ""
        if chosen_reps is not None and ci in chosen_reps:
            rep_str = f"  (representative chosen: {chosen_reps[ci] + 1})"
        print(f"  Class {ci}: size={len(cls)} type={type_label} atoms={atoms_1indexed}{rep_str}{flag}")
...
def choose_best_representatives(mol, molgraph, carbon_classes):
    """
    Choose one representative atom per symmetry class, using the full
    validated pipeline (Group Steiner Tree baseline -> graph automorphism
    enumeration of every structurally valid alternative -> side-consistency
    scoring), rather than an arbitrary per-class choice.

    This matters specifically for molecules with more than one independent
    symmetric block/arm: choosing a representative independently per class
    (e.g. always the lowest atom index) can scatter representatives across
    different physical copies rather than keeping them on one consistent
    side/block, even though every choice is individually valid. This
    function returns the best-scoring (most "one-sided") of all the
    structurally valid alternatives.

    Returns
    -------
    chosen_reps : dict
        class_idx -> representative atom idx, for the single
        best-scoring combination.
    """
    tree_nodes, chosen_reps = greedy_group_steiner_tree(molgraph, carbon_classes)
    combos = enumerate_all_symmetric_combinations(
        mol, molgraph, carbon_classes, baseline_tree_nodes=tree_nodes, baseline_chosen_reps=chosen_reps
    )
    scored = rank_combinations_by_side_consistency(mol, molgraph, carbon_classes, combos)
    return scored[0][1]["chosen_reps"]
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Comma-separated list of 1-indexed atom numbers (matching molfile "
        "numbering) to manually split out of their current symmetry class, "
        "e.g. because experimental DEPT/HSQC data shows more distinct peaks "
        "than predicted for that carbon type. Example: --split-atoms 12,45",
    )
    ...
    if args.split_atoms:
        split_atoms_1indexed = [int(x.strip()) for x in args.split_atoms.split(",") if x.strip()]
        split_atoms_0indexed = [a - 1 for a in split_atoms_1indexed]
        carbon_classes = split_symmetry_class(carbon_classes, split_atoms_0indexed)
        print(f"Manually split atoms {split_atoms_1indexed} out of their original classes.")
        print()
    ...
    print()
    print_class_summary(mol, carbon_classes, chosen_reps)
    print()
